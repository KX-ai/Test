...
{long_text[:2048]}"""  # Summarizing only the first 2048 characters

    try:
        outputs = generator(prompt, do_sample=True, max_new_tokens=100)
...
def query_gpt_neox(text_prompt):
    if not generator:
        return "Model could not be loaded. Please try again later."
    try:
        outputs = generator(text_prompt, do_sample=True, max_new_tokens=100)  # Limit output tokens
        return outputs[0]["generated_text"].strip()
    except Exception as e:
...
def chat_with_document(document_text, user_input):
    # Chunk the document text if it's too long
    chunks = chunk_text(document_text)
    if len(chunks) > 1:
        st.warning(f"Document is too long, splitting it into {len(chunks)} chunks.")

    # Process each chunk and summarize or interact with the user
    responses = []
    for chunk in chunks:
        prompt = (
        ...
        response = query_gpt_neox(prompt)
