...
# Function to load the GPT-NeoX model using the pipeline (using CPU only)
@st.cache_resource
def load_model():
    try:
        # Explicitly use CPU, remove CUDA handling
        device = -1  # Use CPU
        st.write(f"Using device: {'CPU'}")
        return pipeline('text-generation', model='EleutherAI/gpt-neox-350m', device=device)
...
# Function to interact with GPT-NeoX
...
def summarize_chunks(chunks):
    return [summarize_text(chunk) for chunk in chunks]
...
def main():
    ...
    if not generator:
        st.error("The GPT-NeoX model could not be loaded. Please restart the app.")
        return
